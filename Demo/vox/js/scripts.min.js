...
$(function() {
  //ориентация мобильных 
  const mql = window.matchMedia("(orientation: portrait)");

  if (mql.matches) {
    $("header, section, footer").removeClass("landscape");
    $(".attention").remove();
  } else {
    $("header, section, footer").addClass("landscape");
    $("body").prepend("<div class='attention'><p class='attention__message'>Переверните устройство в портретный режим ориентации</p></div>");
  }
...
*/  
   ...
   //scrollTop
    const speed = 550,
/*    $scrollTop = $('<a href="#" class="scrollTop" title="ВВЕРХ"><i class="far fa-angle-up"></i></a>').appendTo("body");
*/
    $scrollTop = $(".scrollTop");
...
/*
    window.document.addEventListener("scroll", function(e) {
      ...
      console.log("ok");
      scrolled > 1000 ? $scrollTop.fadeIn(700) : $scrollTop.fadeOut(700);
    });
 */   
  //input
  ...
  const pentasHoverE = new TimelineMax({paused: true});
  const pentasHoverO = new TimelineMax({paused: true});
   //const pentasHover = new TimelineMax({paused: true});

  pentasE
  .staggerFromTo(".pent:even", 4, {x: 5, y: 10, rotation: 0, strokeOpacity: 0.5}, { ease: Back.easeInOut.config(1.5), x: -10, y: -5, rotation: -5, strokeOpacity: 0.8, repeat: -1, yoyo: true }, 0.4, "0");

  pentasO
  .staggerFromTo(".pent:odd", 4, {x: -5, y: -10, rotation: 0, strokeOpacity: 0.5}, { ease: Back.easeInOut.config(1.5), x: 10, y: 5, rotation: 5, strokeOpacity: 0.8, repeat: -1, yoyo: true }, 0.4, "0");

  pentasHoverE
  .staggerTo(".pent:even", 0.3, { x: -12, y: -20, ease: Circ.easeOut, scale: 0.9}, 0.01, "0");

  pentasHoverO
    .staggerTo(".pent:odd", 0.3, { x: 12, y: 20, ease: Circ.easeOut, scale: 0.9 }, 0.01, "0");
  ...
  $(".pentas").hover(
    ...
    function() {
      pentasHoverE.progress(0).stop();
      pentasHoverO.progress(0).stop();
    });
   ...
   //menu two level 
    ...
    .addTo(ctrl)
    .setClassToggle('.scrollTop', 'active')
    .setTween(workAnim);
  ...
  //custom select
    ...
    if (window.innerWidth < 801) {
      $(".landscape").css({ filter: "blur(7px)" });
      $(".attention").fadeIn(300);
    }

    if(window.innerWidth < 769) {
      benIc.progress(3).pause();
      workAnim.progress(2).pause();
      source.progress(2).pause();
      sourceContent.progress(3).pause();
      invite.progress(5).pause();
    }
    ...
    $(window).resize(function(){
      ...
      if (window.innerWidth < 769) {
        ...
        benIc.progress(0).resume();
        workAnim.progress(0).resume();
        source.progress(0).resume();
        sourceContent.progress(0).resume();
        invite.progress(0).resume();
      }
      if (window.innerWidth < 576) {
        sm.pause();
      } else {
